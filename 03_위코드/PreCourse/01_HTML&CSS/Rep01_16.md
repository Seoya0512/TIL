...
이미지를 웹 사이트에 추가하는 방법이 몇 개 있는데 가장 많이 쓰이는 방법은 img 태그를 사용하는 것
...
또 다른 이미지 넣는 방법은 태그가 아니라 css를 이용해 이미지를 생성하는 것

일단 이미지를 넣을 영역이 필요해서 div 태그를 하나 추가
